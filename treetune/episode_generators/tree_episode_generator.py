...
class TreeEpisodeUtils:
    ...
    def extract_edges_from_tree(
        ...
        def dfs(node, parent=None):
            ...
            if parent is not None: # For each node in the tree except the root
                query_text = parent["full_text"]
                response_text = node["text"]
                parent_reward = parent["reward"]
                assert "reward_std" in parent
                parent_reward_std = parent["reward_std"]
                child_reward = node["reward"]
                leaf = node["leaf"]
                ...
                # if parent_reward != 0 and parent_reward != 1: # parent reward = 0 means there is no difference
                # if advantage != 0:
