...
@EpisodeGenerator.register("math_episode_generator_w_mc_advantages_naive")
class MathEpisodeGeneratorWithMCAdvantagesNaive(MathEpisodeGenerator):
    def __init__(
        self,
    ...
    def _create_value_estimation_requests(
        ...
        for traj_idx, traj in enumerate(trajectories):
            ...
            def get_cutpoints(lst, k):
                ...
                current_end = -1
                for i in range(k):
                    length = base + 1 if i < rem else base
                    current_end += length
                    cutpoints.append(current_end)
                cutpoints = cutpoints[:-1]
                return cutpoints

            cutpoints = get_cutpoints(traj["response_token_ids"], 4)
