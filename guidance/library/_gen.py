...
def _extract_choice_logprobs(choice, logprobs):
    if logprobs is None:
        return [], []
    payload = choice.get("logprobs")
    if payload is None:
        raise RuntimeError("Requested generation logprobs, but LLM response did not include logprobs")
    key = "token_logprobs" if int(logprobs) == 1 else "top_logprobs"
    values = payload.get(key)
    if values is None:
        raise RuntimeError(f"Requested generation logprobs, but response logprobs missing {key}")
    tokens = payload.get("tokens") or []
    return list(values), list(tokens)
    ...
    if n == 1:
        ...
        if list_append:
            ...
            if logprobs is not None:
                ...
                variable_stack[name + "_tokens"] = variable_stack.get(
                    name + "_tokens", []
                )
                variable_stack[name + "_logprobs"].append([])
                variable_stack[name + "_tokens"].append([])
                assert len(variable_stack[name]) == len(variable_stack[name + "_logprobs"])
        async for resp in gen_obj:
            ...
            if logprobs is not None:
                choice_logprobs, choice_tokens = _extract_choice_logprobs(
                    resp["choices"][0], logprobs
                )
                logprobs_out.extend(choice_logprobs)
                logprobs_tokens.extend(choice_tokens)
            if list_append:
                variable_stack[name][list_ind] = generated_value
                if logprobs is not None:
                    variable_stack[name + "_logprobs"][list_ind] = logprobs_out
                    variable_stack[name + "_tokens"][list_ind] = logprobs_tokens
            elif name is not None:
                variable_stack[name] = generated_value
                if logprobs is not None:
                    variable_stack[name + "_logprobs"] = logprobs_out
                    variable_stack[name + "_tokens"] = logprobs_tokens
        ...
        if list_append:
            ...
            variable_stack[name] = generated_value
            variable_stack[name + "_finish_reason"] = resp["choices"][0].get(
                "finish_reason", None
            )
        ...
        if logprobs is not None:
            ...
            for choice in gen_obj["choices"]:
                values, tokens = _extract_choice_logprobs(choice, logprobs)
                logprob_values.append(values)
                logprob_tokens.append(tokens)
            if int(logprobs) == 1:
                for values, tokens in zip(logprob_values, logprob_tokens):
                    assert not tokens or len(values) == len(tokens)
        ...
        if list_append:
            ...
            if logprobs is not None:
                ...
                tokens_list = variable_stack.get(name + "_tokens", [])
                logprobs_list.append(logprob_values)
                tokens_list.append(logprob_tokens)
                variable_stack[name + "_logprobs"] = logprobs_list
                variable_stack[name + "_tokens"] = tokens_list
        elif name is not None:
            variable_stack[name] = generated_values
            finish_reasons = [
                choice.get("finish_reason", None) for choice in gen_obj["choices"]
            ]
            variable_stack[name + "_finish_reason"] = finish_reasons
            if logprobs is not None:
                variable_stack[name + "_logprobs"] = logprob_values
                variable_stack[name + "_tokens"] = logprob_tokens
